  ...
  if (stringComplete) {
    ...
    do {
      ...
      if (inputString.startsWith(STEPS_COMMAND)) {
        String steps_num = inputString.substring(sizeof(STEPS_COMMAND) - 1);
        if (stepsPending != 0)
        {
          parseOk = false;
          break;
        }
        dist = steps_num.toInt();
        currentCommand = MOVE;
        break;
      }

      if (inputString.startsWith(SPEED_COMMAND))
      {
        String speed_num = inputString.substring(sizeof(SPEED_COMMAND)-1);
        stepdelay = speed_num.toInt();
        currentCommand = IDLE;
        break;
      }
